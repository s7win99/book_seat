...
from schemas import SeatOut, SeatPublic
...
@router.get("", response_model=list[SeatPublic])
...
def get_seat_by_token(token: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    user_has_fixed = db.query(Seat).filter(
        Seat.seat_type == "fixed",
        Seat.assigned_user_id == current_user.id,
    ).first() is not None
    ...
    return {
        ...
        "user_has_fixed_seat": user_has_fixed,
    }
